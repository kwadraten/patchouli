...
public sealed class PdfPageRenderingTests
{
    ...
    [Fact] public void README_states_batch_pdf_ocr_not_implemented() => File.ReadAllText(TestPaths.FromRepositoryRoot("README.md")).Should().Contain("batch PDF OCR");
    [Fact] public void KnownIssues_mentions_pdf_renderer_limitations() => File.ReadAllText(TestPaths.FromRepositoryRoot("docs","KNOWN_ISSUES_ALPHA.md")).Should().Contain("Poppler `pdftoppm`");
