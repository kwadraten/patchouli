...
public sealed class SnapshotTests
{
    ...
    [Fact] public async Task Snapshot_redacts_local_ocr_model_path_from_data_shard() { const string path = "/opt/homebrew/bin/tesseract"; await using var c = await SnapshotTestContext.CreateAsync(); var presets = new OcrPresetService(c.Database.ConnectionFactory, new LibraryIdentityService(c.Database.ConnectionFactory, new LiteratureApp.Core.Time.SystemClock()), new LiteratureApp.Core.Time.SystemClock()); await presets.CreatePresetAsync("Local", null, OcrEngineIds.TesseractCli, OcrModelIds.TesseractDefault, path, "{}", true); var published = await c.PublishAsync(); var shardPath = Path.Combine(c.SyncRoot, published.Value.Shards.Single().FileName); await using var shard = OpenShard(c.SyncRoot, published.Value.Shards.Single()); (await shard.ExecuteScalarAsync<string>("select model_path from ocr_preset_versions limit 1;")).Should().Be("[redacted]"); (await File.ReadAllTextAsync(shardPath)).Should().NotContain(path); }
    ...
    [Fact] public async Task Imported_staging_db_can_open_and_read_library_metadata() { await using var c = await SnapshotTestContext.CreateAsync(); var r = await c.PublishAsync(); var imported = await c.ImportAsync(r.Value.ManifestPath); await using var cn = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = imported.Value.StagingDatabasePath }.ToString()); await cn.OpenAsync(); (await cn.ExecuteScalarAsync<string>("select library_id from library_metadata limit 1;")).Should().Be(c.LibraryId.ToString()); }
    [Fact] public async Task Imported_staging_db_can_read_item_and_evidence_records() { await using var c = await SnapshotTestContext.CreateAsync(); var ev = await c.Evidence.CreateFromSearchUnitAsync(c.UnitId); var r = await c.PublishAsync(); var imported = await c.ImportAsync(r.Value.ManifestPath); await using var cn = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = imported.Value.StagingDatabasePath }.ToString()); await cn.OpenAsync(); (await cn.ExecuteScalarAsync<int>("select count(1) from items;")).Should().Be(1); (await cn.ExecuteScalarAsync<string>("select evidence_ref_id from evidence_ref_records limit 1;")).Should().Be(ev.Value.EvidenceRefId); }
    ...
    private static SqliteConnection OpenShard(string syncRoot, SnapshotShard shard) { var cn = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = Path.Combine(syncRoot, shard.FileName) }.ToString()); cn.Open(); return cn; }
