...
public sealed class TaskObservationExtensionsTests
{
    ...
    {
        TaskCompletionSource<(Exception Exception, string Boundary, string? Operation)> reported = new();
        ...
        (Exception Exception, string Boundary, string? Operation) report =
    ...
    public async Task Observe_ignores_cancellation_when_supplied_token_is_canceled()
    {
        ...
        cancellation.Cancel();
        bool reported = false;
        RecordingUnexpectedExceptionSink sink = new((_, _, _) => reported = true);
    ...
    public async Task Observe_reports_cancellation_when_supplied_token_is_not_canceled()
    {
        ...
        RecordingUnexpectedExceptionSink sink = new((exception, _, _) => reported.SetResult(exception));
