...
public sealed class SnapshotTests
{
    ...
    [Fact] public async Task PublishSnapshot_splits_data_shards_when_runtime_exceeds_target_size() { await using var c = await SnapshotTestContext.CreateAsync(); var r = await c.PublishAsync(targetShardSizeBytes: 1); r.Value.Shards.Should().HaveCountGreaterThan(1); foreach (var shard in r.Value.Shards) (await SnapshotPublisher.VerifyShardAsync(c.SyncRoot, shard)).Should().BeTrue(); var imported = await c.ImportAsync(r.Value.ManifestPath); await using var cn = OpenSqlite(imported.Value.StagingDatabasePath!); await cn.OpenAsync(); (await cn.ExecuteScalarAsync<int>("select count(1) from items;")).Should().Be(1); (await cn.ExecuteScalarAsync<int>("select count(1) from pages;")).Should().Be(1); (await cn.ExecuteScalarAsync<int>("select count(1) from search_units;")).Should().Be(1); }
    [Fact] public async Task PublishSnapshot_row_splits_oversized_data_tables() { await using var c = await SnapshotTestContext.CreateAsync(); await c.AddSearchUnitCopiesAsync(12, new string('x', 8192)); var r = await c.PublishAsync(targetShardSizeBytes: 4096); r.Value.Shards.Should().Contain(s => s.Kind.StartsWith("data:04:01:", StringComparison.Ordinal)); var imported = await c.ImportAsync(r.Value.ManifestPath); await using var cn = OpenSqlite(imported.Value.StagingDatabasePath!); await cn.OpenAsync(); (await cn.ExecuteScalarAsync<int>("select count(1) from search_units;")).Should().Be(13); }
    ...
    [Fact] public async Task Snapshot_redacts_local_ocr_model_path_from_data_shard() { const string path = "/opt/local/ocr/model"; await using var c = await SnapshotTestContext.CreateAsync(); var presets = new OcrPresetService(c.Database.ConnectionFactory, new LibraryIdentityService(c.Database.ConnectionFactory, new Patchouli.Core.Time.SystemClock()), new Patchouli.Core.Time.SystemClock()); await presets.CreatePresetAsync("Local", null, OcrEngineIds.LocalPlaceholder, "local-model", path, "{}", true); var published = await c.PublishAsync(); var shardPath = Path.Combine(c.SyncRoot, published.Value.Shards.Single().FileName); await using (var shard = OpenShard(c.SyncRoot, published.Value.Shards.Single())) { (await shard.ExecuteScalarAsync<string>("select model_path from ocr_preset_versions limit 1;")).Should().Be("[redacted]"); } (await File.ReadAllTextAsync(shardPath)).Should().NotContain(path); }
    ...
    private static string TempDir() => Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"patchouli-snap-{Guid.NewGuid():N}")).FullName;
    ...
    private sealed class SnapshotTestContext : IAsyncDisposable
    {
        ...
        public async Task AddSearchUnitCopiesAsync(int count, string text) { await using var cn = OpenRuntime(); var t = await cn.QuerySingleAsync<SearchUnitTemplate>("select document_instance_id as DocumentInstanceId, page_id as PageId, root_node_id as RootNodeId, text_revision_id as TextRevisionId, bbox_revision_id as BboxRevisionId, layout_revision_id as LayoutRevisionId, node_type as NodeType from search_units limit 1;"); for (var i = 0; i < count; i++) await cn.ExecuteAsync("insert into search_units (unit_id, document_instance_id, page_id, root_node_id, text_revision_id, bbox_revision_id, layout_revision_id, resolved_text, bbox_union_json, node_type, reading_order, status, supersedes_unit_id, superseded_by_unit_id, created_at, updated_at) values (@UnitId,@DocumentInstanceId,@PageId,@RootNodeId,@TextRevisionId,@BboxRevisionId,@LayoutRevisionId,@Text,null,@NodeType,@ReadingOrder,'active',null,null,@Now,@Now);", new { UnitId = SearchUnitId.New().ToString(), t.DocumentInstanceId, t.PageId, t.RootNodeId, t.TextRevisionId, t.BboxRevisionId, t.LayoutRevisionId, Text = text, t.NodeType, ReadingOrder = i + 2, Now = "2026-01-01T00:00:00.0000000Z" }); }
        ...
        private sealed record SearchUnitTemplate(string DocumentInstanceId, string PageId, string RootNodeId, string TextRevisionId, string BboxRevisionId, string LayoutRevisionId, string NodeType);
    }
}
