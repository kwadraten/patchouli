...
public sealed class PdfPageRenderingTests
{
    ...
    [Fact] public void Product_ui_uses_pdf_preview_placeholder_not_legacy_batch_ocr_copy() => File.ReadAllText(TestPaths.FromRepositoryRoot("src","Patchouli.UI","Views","PdfWorkspacePage.axaml")).Should().Contain("工作台预览").And.NotContain("batch PDF OCR");
    ...
    {
        private Context(TemporarySqliteDatabase db, FixedClock clock, LibraryId libraryId, DocumentInstanceId doc, PageId page, FileAssetId asset, string pdf, string cache, string sync, PageRenderService rendererService, CountingRenderer renderer, PageService pages, McpReadApi mcp)
        { Database=db;Clock=clock;LibraryId=libraryId;DocumentInstanceId=doc;PageId=page;AssetId=asset;PdfPath=pdf;CacheRoot=cache;SyncRoot=sync;RenderService=rendererService;Renderer=renderer;Pages=pages;Mcp=mcp; }
        public TemporarySqliteDatabase Database{get;} public FixedClock Clock{get;} public LibraryId LibraryId{get;} public DocumentInstanceId DocumentInstanceId{get;} public PageId PageId{get;} public FileAssetId AssetId{get;} public string PdfPath{get;} public string CacheRoot{get;} public string SyncRoot{get;} public PageRenderService RenderService{get;} public CountingRenderer Renderer{get;} public PageService Pages{get;} public McpReadApi Mcp{get;}
        public static async Task<Context> CreateAsync(string extension=".pdf")
        {
            var db=TemporarySqliteDatabase.Create(); var clock=new FixedClock(DateTimeOffset.Parse("2026-06-20T00:00:00Z")); var dir=Directory.CreateDirectory(Path.Combine(Path.GetTempPath(),$"render-{Guid.NewGuid():N}")).FullName; var pdf=Path.Combine(dir,"source"+extension); File.Copy(TestFixtures.RealThreePagePdf,pdf); var cache=Path.Combine(dir,"cache"); var sync=Path.Combine(dir,"sync");
            ...
            return new Context(db,clock,lib.Value.LibraryId,doc.Value.DocumentInstanceId,page.Value.PageId,asset.Value.FileAssetId,pdf,cache,sync,renderService,renderer,pages,new McpReadApi(db.ConnectionFactory,new SqliteSearchService(db.ConnectionFactory),new EvidenceReferenceService(db.ConnectionFactory,clock))){ RootDirectory=dir };
        }
        ...
        public async ValueTask DisposeAsync(){await Database.DisposeAsync();if(Directory.Exists(RootDirectory))Directory.Delete(RootDirectory,true);}
    }
