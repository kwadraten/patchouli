...
public sealed class PdfPageRenderingTests
{
    ...
    [Fact] public async Task RunPresetOnRenderedPdfPage_source_changed_creates_skipped_page_result() { await using var c=await Context.CreateAsync(); await File.AppendAllTextAsync(c.PdfPath,"changed"); var r=await c.Coordinator.RunPresetOnRenderedPdfPageAsync(c.DocumentInstanceId,c.PresetId,c.PageId); r.IsSuccess.Should().BeTrue(); r.Value.State.Should().Be(OcrRunState.CompletedWithErrors); var page=(await c.Coordinator.ListPageResultsAsync(r.Value.OcrRunId)).Value.Single(); page.State.Should().Be(OcrPageResultState.Skipped); page.ErrorCode.Should().Be(OcrFailureCode.SourceFileChanged); (await c.CountAsync("layout_nodes")).Should().Be(0); }
