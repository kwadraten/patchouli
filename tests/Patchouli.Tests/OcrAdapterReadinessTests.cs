...
{
    [Fact]
    public async Task ModelPathValidator_required_missing_returns_missing_model_path()
    {
        (await new OcrModelPathValidator().ValidateModelPathAsync(null, true)).Status.Should()
            .Be(OcrEnvironmentStatus.MissingModelPath);
    }
    ...
    public async Task ModelPathValidator_existing_file_returns_ready()
    {
        ...
        try
        {
            (await new OcrModelPathValidator().ValidateModelPathAsync(path, true)).IsReady.Should().BeTrue();
        }
        finally
        {
            File.Delete(path);
    ...
    {
        string path = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"ocr-model-{Guid.NewGuid():N}"))
            .FullName;
        try
        {
            (await new OcrModelPathValidator().ValidateModelPathAsync(path, true)).IsReady.Should().BeTrue();
    ...
    [Fact]
    public async Task ModelPathValidator_missing_path_returns_inaccessible()
    {
        (await new OcrModelPathValidator().ValidateModelPathAsync(
                Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N")), true)).Status.Should()
            .Be(OcrEnvironmentStatus.ModelPathInaccessible);
    }

    [Fact]
    public async Task ModelPathValidator_valid_http_url_is_accepted_without_network_call()
    {
        (await new OcrModelPathValidator().ValidateModelPathAsync("https://example.invalid/model", true)).IsReady
            .Should().BeTrue();
    }
    ...
    {
        await using OcrReadinessContext c = await OcrReadinessContext.CreateAsync();
        Result<OcrPresetVersion> original = await c.Presets.GetCurrentVersionAsync(c.PresetId);
        Result<OcrPresetVersion> rebound = await c.Presets.RebindModelPathAsync(c.PresetId, "/not-yet-present/model");
    ...
    {
        await using OcrReadinessContext c = await OcrReadinessContext.CreateAsync();
        Result<OcrPresetVersion> old = await c.Presets.GetCurrentVersionAsync(c.PresetId);
        ...
        (await connection.ExecuteScalarAsync<string?>(
            "select model_path from ocr_preset_versions where preset_version_id = @Id;",
            new { Id = old.Value.PresetVersionId.ToString() })).Should().Be(old.Value.ModelPath);
    }

    [Fact]
    public async Task RebindModelPath_rejects_blank_path()
    {
        await using OcrReadinessContext c = await OcrReadinessContext.CreateAsync();
        (await c.Presets.RebindModelPathAsync(c.PresetId, " ")).ErrorCode.Should().Be(AppErrorCodes.ValidationFailed);
    }

    [Fact]
    public void AdapterRegistry_lists_mock_capability()
    {
        CreateRegistry().ListCapabilities().Should().Contain(c => c.EngineId == OcrEngineIds.Mock);
    }

    [Fact]
    public void AdapterRegistry_lists_local_placeholder_capability()
    {
        CreateRegistry().ListCapabilities().Should()
            .Contain(c => c.EngineId == OcrEngineIds.LocalPlaceholder && c.RequiresModelPath);
    }

    [Fact]
    public async Task LocalPlaceholder_CheckEnvironment_missing_model_path_not_ready()
    {
        LocalPlaceholderOcrAdapter adapter = new(new OcrModelPathValidator());
        OcrEnvironmentCheckResult result = await adapter.CheckEnvironmentAsync(
            new OcrPresetVersion(OcrPresetVersionId.New(), OcrPresetId.New(), OcrEngineIds.LocalPlaceholder,
                "local-model", null, "{}", false, DateTimeOffset.UtcNow));
    ...
    {
        string path = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"ocr-local-{Guid.NewGuid():N}"))
            .FullName;
        try
        {
            LocalPlaceholderOcrAdapter adapter = new(new OcrModelPathValidator());
            (await adapter.CheckEnvironmentAsync(new OcrPresetVersion(OcrPresetVersionId.New(), OcrPresetId.New(),
                    OcrEngineIds.LocalPlaceholder, "local-model", path, "{}", false, DateTimeOffset.UtcNow))).IsReady
                .Should().BeTrue();
        }
        finally
        {
    ...
    {
        await using OcrReadinessContext c = await OcrReadinessContext.CreateAsync();
        Result<OcrRun> result = await c.Coordinator.RunPresetOnPagesAsync(c.DocumentInstanceId, c.PresetId, [c.PageId]);
    ...
    {
        await using OcrReadinessContext c = await OcrReadinessContext.CreateAsync(OcrEngineIds.LocalPlaceholder, null);
        Result<OcrRun> result = await c.Coordinator.RunPresetOnPagesAsync(c.DocumentInstanceId, c.PresetId, [c.PageId]);
    ...
    {
        string pdfPath = Path.Combine(Path.GetTempPath(), $"mineru-preset-{Guid.NewGuid():N}.pdf");
        ...
        try
        {
            ...
            Result<OcrRun> result = await c.Coordinator.RunPresetOnDocumentAsync(c.DocumentInstanceId, c.PresetId);
    ...
    {
        FakeRegionAdapter adapter = new();
        await using OcrReadinessContext c = await OcrReadinessContext.CreateAsync(FakeRegionAdapter.EngineIdValue,
            adapter: adapter, applyOnSuccess: true);

        Result<OcrRun> result = await c.Coordinator.RunPresetOnRegionAsync(c.DocumentInstanceId, c.PresetId, c.PageId,
            new NormalizedBBox(0.2, 0.2, 0.3, 0.3));
    ...
    {
        await using OcrReadinessContext c =
            await OcrReadinessContext.CreateAsync(FakeRegionAdapter.EngineIdValue, adapter: new FakeRegionAdapter());

        Result<OcrRun> result = await c.Coordinator.RunPresetOnRegionAsync(c.DocumentInstanceId, c.PresetId, c.PageId,
            new NormalizedBBox(0.9, 0.9, 0.2, 0.2));
    ...
    public async Task MCP_does_not_expose_model_path_after_rebind()
    {
        ...
        Result<McpDocumentStatusResponse> result = await c.Mcp.GetDocumentStatusAsync(c.DocumentInstanceId);
    ...
    [Fact]
    public void Step_14A_has_only_reported_table_cell_metadata_migration()
    {
        Directory.EnumerateFiles(TestPaths.MigrationsDirectory, "*.sql").Select(Path.GetFileName)
            .Where(name => name!.Contains("14", StringComparison.OrdinalIgnoreCase)).Should()
            .Equal("014_add_table_cell_metadata.sql");
    }

    private static OcrAdapterRegistry CreateRegistry(IRealOcrAdapter? extraAdapter = null)
    {
        OcrAdapterRegistry registry = new();
        ...
        if (extraAdapter is not null)
        {
            registry.RegisterAdapter(extraAdapter);
        }
    ...
    {
        private OcrReadinessContext(TemporarySqliteDatabase database, OcrPresetService presets,
            OcrRunCoordinator coordinator, McpReadApi mcp, OcrPresetId presetId, DocumentInstanceId documentInstanceId,
            PageId pageId, LayoutRevisionId initialCurrentRevisionId)
        {
            Database = database;
            Presets = presets;
            ...
            PageId = pageId;
            InitialCurrentRevisionId = initialCurrentRevisionId;
        }
        ...
        public static async Task<OcrReadinessContext> CreateAsync(string engineId = OcrEngineIds.Mock,
            string? modelPath = null, IRealOcrAdapter? adapter = null, bool applyOnSuccess = false,
            string? sourcePdfPath = null, int pageCount = 1,
            Func<MinerUConfiguration, IMinerUClient>? minerUClientFactory = null)
        {
            ...
            await library.CreateLibraryAsync("OCR readiness");
            Result<ItemMetadata> item =
                await new ItemService(database.ConnectionFactory, library, clock).CreateItemAsync("book",
                    "OCR readiness item");
            FileAssetId? fileAssetId = null;
            if (!string.IsNullOrWhiteSpace(sourcePdfPath))
            {
                fileAssetId =
                    (await new FileAssetService(database.ConnectionFactory, library, clock).RegisterFileAsync(
                        sourcePdfPath)).Value.FileAssetId;
            }
            ...
            {
                Result<Page> page = await new PageService(database.ConnectionFactory, clock).CreatePageAsync(
                    document.Value.DocumentInstanceId, i, (i + 1).ToString(), null, null, 0,
            ...
            Result<LayoutRevision> currentRevision =
                await layout.CreateLayoutRevisionAsync(document.Value.DocumentInstanceId, LayoutRevisionSource.Manual,
                    true);
            OcrPresetService presets = new(database.ConnectionFactory, library, clock);
    ...
    private sealed class FakeRegionAdapter : IRealOcrAdapter
    {
        ...
        {
            return new OcrEngineCapability(EngineId, DisplayName, false, false, false, false, true, false, false, false,
                false,
                [OcrInputKinds.RegionImage], "Test adapter for region OCR.");
        }

        public Task<OcrEnvironmentCheckResult> CheckEnvironmentAsync(OcrPresetVersion presetVersion,
            CancellationToken cancellationToken = default)
        {
            return Task.FromResult(new OcrEnvironmentCheckResult(EngineId, presetVersion.ModelId,
                presetVersion.ModelPath, OcrEnvironmentStatus.Ready, true, "ready", OcrRequiredAction.None, []));
        }
        ...
        {
            return Task.FromResult(input.InputKind == OcrInputKinds.RegionImage && input.RegionBBox is not null
                ? Result.Success()
                : Result.Failure(AppErrorCodes.ValidationFailed, "region bbox required"));
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return Task.FromResult(Result<OcrEnginePageResult>.Success(
                new OcrEnginePageResult(input.PageId, true, "region text", input.RegionBBox, null, null)));
        }
    }

    private static string CreateMinerUZip(string contentListJson)
    {
        string zipPath = Path.Combine(Path.GetTempPath(), $"mineru-preset-{Guid.NewGuid():N}.zip");
        ...
        return new MinerUClient(
            new HttpClient(new MinerUProtocolHandler(request =>
            {
                if (request.Method == HttpMethod.Post && request.RequestUri!.AbsolutePath == "/api/v4/file-urls/batch")
                {
                    return new HttpResponseMessage(System.Net.HttpStatusCode.OK)
                    {
                        Content = new StringContent(
                            """{"code":0,"data":{"batch_id":"batch-1","file_urls":["https://upload.example.test/file"]},"msg":"ok"}""")
                    };
                }
                ...
                if (request.Method == HttpMethod.Get &&
                    request.RequestUri!.AbsolutePath == "/api/v4/extract-results/batch/batch-1")
                {
                    return new HttpResponseMessage(System.Net.HttpStatusCode.OK)
                    {
                        Content = new StringContent("""
