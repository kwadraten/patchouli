...
{
    [Fact] public void BuildInfo_exposes_version() { BuildInfo.AppName.Should().Be("Patchouli.Net"); BuildInfo.Version.Should().Be("0.1.0"); BuildInfo.SchemaVersion.Should().Be(AppSchemaVersion.Current); }
    ...
    [Fact] public void UI_version_info_viewmodel_exposes_version_schema_db_path() { var vm=new MainWindowViewModel(new TestClipboard()); vm.VersionInfo.Should().Contain("0.1.0").And.Contain("Schema").And.Contain(vm.RuntimeDatabasePath); }
    ...
    [Fact] public void BuildInfo_has_no_prerelease_suffix() { BuildInfo.Version.Should().Be("0.1.0"); }
