...
public sealed class EvidenceReferenceServiceTests
{
    ...
    [Fact] public async Task Tombstone_hides_search_fts_and_layout_but_keeps_payload_marker() { await using var c = await EvidenceTestContext.CreateWithUnitAsync("Tombstone payload"); await c.Rebuilder.RebuildFtsForLibraryAsync(); (await c.Search.SearchLibraryAsync(new SearchRequest("Tombstone"))).Value.Results.Should().NotBeEmpty(); var record = await c.Evidence.CreateFromSearchUnitAsync(c.UnitId); await c.Evidence.TombstoneAsync(record.Value.EvidenceRefId, "removed"); (await c.Evidence.ResolveAsync(record.Value.EvidenceRefId)).Value.Status.Should().Be(EvidenceResolutionStatus.Tombstoned); (await c.Search.SearchLibraryAsync(new SearchRequest("Tombstone"))).Value.Results.Should().BeEmpty(); (await c.SearchUnitStatusAsync()).Should().Be(SearchUnitStatus.Hidden); (await c.CountFtsRowsAsync()).Should().Be(0); (await c.RootNodeIgnoredAsync()).Should().BeTrue(); (await c.RootNodeTextAsync()).Should().Be("Tombstone payload"); (await c.StatusAsync(SearchIndexScopeType.DocumentInstance, c.DocumentInstanceId.ToString())).Should().Be(SearchIndexStatusValue.Stale); }
    ...
    [Fact] public async Task Purge_hides_search_and_clears_layout_search_and_evidence_payloads() { await using var c = await EvidenceTestContext.CreateWithUnitAsync("Sensitive payload"); await c.Rebuilder.RebuildFtsForLibraryAsync(); var record = await c.Evidence.CreateFromSearchUnitAsync(c.UnitId); await c.Evidence.PurgeAsync(record.Value.EvidenceRefId, "purge"); var resolved = await c.Evidence.ResolveAsync(record.Value.EvidenceRefId); resolved.Value.Status.Should().Be(EvidenceResolutionStatus.Purged); resolved.Value.PinnedText.Should().Be("[purged]"); (await c.Search.SearchLibraryAsync(new SearchRequest("Sensitive"))).Value.Results.Should().BeEmpty(); (await c.SearchUnitStatusAsync()).Should().Be(SearchUnitStatus.Deleted); (await c.SearchUnitTextAsync()).Should().Be("[purged]"); (await c.SearchUnitBboxAsync()).Should().BeNull(); (await c.CountFtsRowsAsync()).Should().Be(0); (await c.RootNodeIgnoredAsync()).Should().BeTrue(); (await c.RootNodeTextAsync()).Should().BeNull(); (await c.RootNodeBboxCountAsync()).Should().Be(0); (await c.CountEvidenceRecordsAsync()).Should().Be(1); }
    ...
    private sealed class EvidenceTestContext : IAsyncDisposable
    {
        ...
        public async Task<string?> SearchUnitStatusAsync()
            => await Connection().ExecuteScalarAsync<string?>("select status from search_units where unit_id=@Unit;", new { Unit = UnitId.ToString() });
        public async Task<string?> SearchUnitTextAsync()
            => await Connection().ExecuteScalarAsync<string?>("select resolved_text from search_units where unit_id=@Unit;", new { Unit = UnitId.ToString() });
        public async Task<string?> SearchUnitBboxAsync()
            => await Connection().ExecuteScalarAsync<string?>("select bbox_union_json from search_units where unit_id=@Unit;", new { Unit = UnitId.ToString() });
        public async Task<int> CountFtsRowsAsync()
            => await Connection().ExecuteScalarAsync<int>("select count(1) from search_units_fts where unit_id=@Unit;", new { Unit = UnitId.ToString() });
        public async Task<bool> RootNodeIgnoredAsync()
            => await Connection().ExecuteScalarAsync<int>("select ignored from layout_nodes where node_id = (select root_node_id from search_units where unit_id=@Unit);", new { Unit = UnitId.ToString() }) == 1;
        public async Task<string?> RootNodeTextAsync()
            => await Connection().ExecuteScalarAsync<string?>("select own_text from layout_nodes where node_id = (select root_node_id from search_units where unit_id=@Unit);", new { Unit = UnitId.ToString() });
        public async Task<int> RootNodeBboxCountAsync()
            => await Connection().ExecuteScalarAsync<int>("select ((bbox_x is not null) + (bbox_y is not null) + (bbox_width is not null) + (bbox_height is not null)) from layout_nodes where node_id = (select root_node_id from search_units where unit_id=@Unit);", new { Unit = UnitId.ToString() });
        public async Task<string?> StatusAsync(string scopeType, string scopeId)
            => await Connection().ExecuteScalarAsync<string?>("select status from search_index_status where scope_type=@ScopeType and scope_id=@ScopeId;", new { ScopeType = scopeType, ScopeId = scopeId });
