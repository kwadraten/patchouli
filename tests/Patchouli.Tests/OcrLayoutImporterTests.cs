...
public sealed class OcrLayoutImporterTests
{
    ...
    {
        await using OcrLayoutImportContext context = await OcrLayoutImportContext.CreateAsync(1);

        Result<OcrLayoutImportResult> result = await context.Importer.ImportRevisionAsync(new OcrLayoutImportRequest(
        ...
        string[] cellTexts = (await connection.QueryAsync<string>(
        ...
        int[] headerFlags = (await connection.QueryAsync<int>(
    ...
    {
        await using OcrLayoutImportContext context = await OcrLayoutImportContext.CreateAsync(2);
        Result<OcrLayoutImportResult> source = await context.Importer.ImportRevisionAsync(new OcrLayoutImportRequest(
            context.DocumentInstanceId,
        ...
        LayoutRevisionId targetRevisionId = LayoutRevisionId.New();
        await using (SqliteConnection connection = context.Database.ConnectionFactory.CreateConnection())
        {
            await connection.OpenAsync();
        ...
        Result<OcrLayoutCopyResult> copy = await context.Importer.CopyPagesAsync(
            new OcrLayoutCopyRequest(source.Value.RevisionId, targetRevisionId, [context.Pages[0].PageId]));

        copy.IsSuccess.Should().BeTrue(copy.ErrorMessage);
        await using SqliteConnection verify = context.Database.ConnectionFactory.CreateConnection();
        await verify.OpenAsync();
        int orphanCount = await verify.ExecuteScalarAsync<int>(
            """
            select count(1)
        ...
        string[] copiedPages = (await verify.QueryAsync<string>(
        ...
        string[] copiedCells = (await verify.QueryAsync<string>(
            "select own_text from layout_nodes where revision_id = @RevisionId and node_type = @NodeType order by row_index, col_index;",
    ...
    private static OcrLayoutDocument CreateTableDocument(Page page)
    {
        return new OcrLayoutDocument([CreateTablePage(page)]);
    }

    private static OcrLayoutPage CreateTablePage(Page page)
    {
        return new OcrLayoutPage(
            page.PageId,
            page.PageIndex,
            page.Width,
            page.Height,
                ...
                new OcrLayoutBlock(
                    ...
                    [
                        new OcrLayoutBlock(
                            ...
                            Children:
                            [
                                new OcrLayoutBlock(LayoutNodeType.TableCell, TextPolicy.Own, 3, "Name",
                                    TableCell: new OcrTableCell(0, 0, 1, 1, true)),
                                new OcrLayoutBlock(LayoutNodeType.TableCell, TextPolicy.Own, 4, "Value",
                                    TableCell: new OcrTableCell(0, 1, 1, 1, true))
                            ]),
                        new OcrLayoutBlock(
                            LayoutNodeType.TableRow,
                            TextPolicy.AggregateChildren,
                            ...
                            Children:
                            [
                                new OcrLayoutBlock(LayoutNodeType.TableCell, TextPolicy.Own, 6, "Pages",
                                    TableCell: new OcrTableCell(1, 0)),
                                new OcrLayoutBlock(LayoutNodeType.TableCell, TextPolicy.Own, 7, "12",
                                    TableCell: new OcrTableCell(1, 1))
                            ])
                    ])
            ]);
    }

    private static OcrLayoutPage CreateParagraphPage(Page page, string text)
    {
        return new OcrLayoutPage(
            page.PageId,
            page.PageIndex,
            ...
            page.Height,
            [
                new OcrLayoutBlock(LayoutNodeType.Paragraph, TextPolicy.Own, 1, text)
            ]);
    }
    ...
    private sealed class OcrLayoutImportContext : IAsyncDisposable
    {
        ...
        public static async Task<OcrLayoutImportContext> CreateAsync(int pageCount)
        {
            ...
            Result<ItemMetadata> item =
                await new ItemService(database.ConnectionFactory, library, clock).CreateItemAsync("book", "OCR layout");
            ...
            PageService pageService = new(database.ConnectionFactory, clock);
            List<Page> pages = new();
            for (int index = 0; index < pageCount; index++)
            {
                pages.Add((await pageService.CreatePageAsync(document.Value.DocumentInstanceId, index, $"{index + 1}",
                    600, 800, 0, CoordinateBasis.NormalizedPage, 600, 800, "test", null)).Value);
            }

            return new OcrLayoutImportContext(database, clock, new OcrLayoutImporter(database.ConnectionFactory, clock),
                document.Value.DocumentInstanceId, pages);
        }
