...
public sealed class McpReadApiTests
{
    ...
    [Fact] public async Task GetPageBlocks_current_returns_evidence_ref_for_search_unit_blocks() { await using var c = await McpTestContext.CreateWithIndexedUnitAsync("block text"); var r = await c.Api.GetPageBlocksAsync(new McpPageBlocksRequest(c.PageId)); r.Value.Blocks.Single().EvidenceRef.Should().StartWith("evref:v1:"); }
    [Fact] public async Task GetPageBlocks_current_returns_null_evidence_ref_for_unindexed_blocks() { await using var c = await McpTestContext.CreateWithIndexedUnitAsync("block text"); await c.AddUnindexedBlockAsync("draft block", 2); var r = await c.Api.GetPageBlocksAsync(new McpPageBlocksRequest(c.PageId)); r.Value.Blocks.Single(b => b.Text == "draft block").EvidenceRef.Should().BeNull(); }
    ...
    [Fact] public async Task GetSearchResultContext_returns_bbox_for_units_with_bbox_union() { await using var c = await McpTestContext.CreateWithIndexedUnitAsync("context text"); var r = await c.Api.GetSearchResultContextAsync(new McpSearchContextRequest(c.UnitId)); r.Value.Units.Single().BBox.Should().Be(new NormalizedBBox(0.1, 0.1, 0.2, 0.2)); }
    ...
    private sealed class McpTestContext : IAsyncDisposable
    {
        ...
        public async Task AddUnindexedBlockAsync(string text, int readingOrder) => await new LayoutTreeService(Database.ConnectionFactory, Clock).AddNodeAsync(RevisionId, PageId, null, LayoutNodeType.Paragraph, new NormalizedBBox(0.45, 0.45, 0.1, 0.05), text, TextPolicy.Own, readingOrder, LayoutNodeSource.Manual);
