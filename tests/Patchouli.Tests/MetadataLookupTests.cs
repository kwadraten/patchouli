...
public sealed class MetadataLookupTests
{
    ...
    public void Identifier_normalization_returns_canonical_scheme_and_value(string scheme, string value, string expectedScheme, string expectedValue)
    {
        var result = IdentifierNormalizer.Normalize(scheme, value);
    ...
    {
        var first = new FakeSource("first", Result<MetadataCandidate>.Failure(MetadataLookupErrorCodes.NotFound, "missing"));
        var second = new FakeSource("second", Result<MetadataCandidate>.Success(new MetadataCandidate("second", Title: "Found")));
        var items = new FakeItemService(CreateItem(title: "Old"));
        var service = new MetadataLookupService(items, new MetadataSourceRegistry([first, second]));

        var result = await service.LookupAndMergeAsync(
            items.Item.ItemId,
    ...
    {
        var existing = CreateItem(
            title: "User's latest title",
            subtitle: "Keep subtitle",
            creators:
            [
                Creator(ItemCreatorRoles.Author, family: "Old Author"),
            ...
            tagsJson: "[\"History\"]",
            collectionsJson: "[\"Do not replace\"]",
            customFieldsJson: "{\"user\":true}",
            identifiers: [Identifier(BuiltInIdentifierSchemes.DOI, "10.1000/example")]);
        var items = new FakeItemService(existing);
        var source = new FakeSource("catalog", Result<MetadataCandidate>.Success(new MetadataCandidate(
            "catalog",
            Title: "Source title",
            Subtitle: " ",
            Creators: [new MetadataCreator(ItemCreatorRoles.Author, Family: "New Author")],
        ...
        var service = new MetadataLookupService(items, new MetadataSourceRegistry([source]));

        var result = await service.LookupAndMergeAsync(existing.ItemId, "doi", "10.1000/example");
        ...
        items.LastUpdate.Creators.Should().ContainSingle(creator => creator.Role == ItemCreatorRoles.Author && creator.Family == "New Author");
        items.LastUpdate.Creators.Should().ContainSingle(creator => creator.Role == ItemCreatorRoles.Editor && creator.Literal == "Keep Editor");
        items.LastUpdate.Dates.Should().ContainSingle(date => date.Role == ItemDateRoles.Issued && date.DatePartsJson == "[[2025,6]]");
        ...
        items.AddedIdentifiers.Should().ContainSingle(identifier => identifier.Scheme == BuiltInIdentifierSchemes.Pmid && identifier.Value == "12345");
    }

    [Fact]
    public void High_confidence_source_type_overwrites_item_type()
    {
        var item = CreateItem(title: "Old");
        var update = MetadataLookupService.CreateUpdate(item, new MetadataCandidate(
            "source",
            Title: "New",
    ...
    {
        var item = CreateItem(title: "Old", tagsJson: "legacy-not-json");

        var update = MetadataLookupService.CreateUpdate(item, new MetadataCandidate("source", Title: "New", Tags: ["remote"]));
    ...
    {
        var source = new SchemeSource(BuiltInIdentifierSchemes.Pmcid);
        var service = new MetadataLookupService(new FakeItemService(CreateItem("Old")), new MetadataSourceRegistry([source]));
    ...
    public async Task Ndl_parses_dublin_core_for_ndlbibid()
    {
        const string xml = """
            ...
            """;
        var handler = new FakeHttpHandler(xml, "application/xml");
        var source = new NdlMetadataSource(new HttpClient(handler));

        var result = await source.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.Ndlbibid, "000002179229"));

        result.IsSuccess.Should().BeTrue(result.ErrorMessage);
        result.Value.Title.Should().Be("在村蘭学の展開");
        result.Value.Identifiers.Should().Contain(identifier => identifier.Scheme == BuiltInIdentifierSchemes.Jpno && identifier.Value == "92042014");
        result.Value.Identifiers.Should().Contain(identifier => identifier.Scheme == BuiltInIdentifierSchemes.ISBN && identifier.Value == "4-7842-0701-5");
    ...
    public async Task Crossref_parses_representative_json_response()
    {
        ...
        var source = new CrossrefMetadataSource(new HttpClient(handler));

        var result = await source.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.DOI, "10.1000/example"));
    ...
    public async Task PubMed_parses_representative_xml_response()
    {
        const string xml = """
            ...
            """;
        var source = new PubMedMetadataSource(new HttpClient(new FakeHttpHandler(xml, "application/xml")));

        var result = await source.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.Pmid, "123"));
        ...
        result.Value.Identifiers.Should().Contain(identifier => identifier.Scheme == BuiltInIdentifierSchemes.DOI && identifier.Value == "10.1000/xml");
    }
    ...
    public async Task Google_books_zero_daily_quota_is_not_reported_as_retryable_rate_limit()
    {
        const string json = """
            ...
            """;
        var source = new GoogleBooksMetadataSource(new HttpClient(new FakeHttpHandler(json, "application/json", HttpStatusCode.TooManyRequests)));

        var result = await source.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.ISBN, "9787209108546"));
    ...
    public async Task Calis_uses_isbn_search_then_maps_unimarc_detail()
    {
        const string marc = """
            ...
            """;
        var search = """{"data":{"instances":[{"rid":"a5440f47f2da16b57a52ea525e41f84b"}]}}""";
        var detail = JsonSerializer.Serialize(new { data = new { MARC = marc } });
        var handler = new RoutingHttpHandler(request => request.Method == HttpMethod.Post
            ? JsonResponse(search)
            : JsonResponse(detail));
        var source = new CalisMetadataSource(new HttpClient(handler));

        var result = await source.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.ISBN, "9787522865270"));
    ...
    public async Task Calis_skips_non_matching_marc_records()
    {
        const string marc = """
            ...
            """;
        var search = """{"data":{"instances":[{"rid":"different"}]}}""";
        var detail = JsonSerializer.Serialize(new { data = new { MARC = marc } });
        var handler = new RoutingHttpHandler(request => request.Method == HttpMethod.Post
            ? JsonResponse(search)
            : JsonResponse(detail));
    ...
    {
        var empty = new CalisMetadataSource(new HttpClient(new FakeHttpHandler("""{"data":{"instances":[]}}""", "application/json")));
        var invalid = new CalisMetadataSource(new HttpClient(new FakeHttpHandler("not json", "application/json")));

        var emptyResult = await empty.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.ISBN, "9787209108546"));
        var invalidResult = await invalid.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.ISBN, "9787209108546"));
    ...
    {
        var calis = new CalisMetadataSource(new HttpClient(new FakeHttpHandler("", "application/json", HttpStatusCode.TooManyRequests)));
        var nlc = new NationalLibraryOfChinaMetadataSource(new HttpClient(new FakeHttpHandler("", "text/html", HttpStatusCode.TooManyRequests)));

        var calisResult = await calis.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.ISBN, "9787209108546"));
        var nlcResult = await nlc.LookupAsync(new NormalizedIdentifier(BuiltInIdentifierSchemes.ISBN, "9787209108546"));
    ...
    public async Task National_library_of_china_maps_aleph_html()
    {
        ...
        var source = new NationalLibraryOfChinaMetadataSource(new HttpClient(handler));
    ...
    public async Task National_library_of_china_rejects_a_non_matching_isbn_record()
    {
        const string html = """
            ...
            """;
        var source = new NationalLibraryOfChinaMetadataSource(new HttpClient(new FakeHttpHandler(html, "text/html")));
    ...
    private static ItemMetadata CreateItem(
        ...
        => new(ItemId.New(), LibraryId.New(), "book", "preserved-key", title, subtitle, null, "[]", creators ?? [], null, dates ?? [], identifiers ?? [], null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, tagsJson, collectionsJson, customFieldsJson, DateTimeOffset.UnixEpoch, DateTimeOffset.UnixEpoch);

    private static ItemCreator Creator(string role, string? family = null, string? literal = null)
        => new(Guid.NewGuid().ToString(), default, role, family, null, literal, null, null, 0, DateTimeOffset.UnixEpoch);
    private static ItemDate Date(string role, string parts)
        => new(Guid.NewGuid().ToString(), default, role, parts, false, null, null, DateTimeOffset.UnixEpoch);
    private static ItemIdentifier Identifier(string scheme, string value)
        => new(IdentifierId.New(), default, scheme, value, null, DateTimeOffset.UnixEpoch);

    private sealed class FakeSource(string id, Result<MetadataCandidate> result) : IMetadataSource
    {
        public MetadataSourceDefinition Definition { get; } = new(id, id, new HashSet<string>(StringComparer.OrdinalIgnoreCase) { BuiltInIdentifierSchemes.DOI }, 10);
        public int CallCount { get; private set; }
        public Task<Result<MetadataCandidate>> LookupAsync(NormalizedIdentifier identifier, CancellationToken cancellationToken = default)
        {
            CallCount++;
    ...
    {
        public MetadataSourceDefinition Definition { get; } = new("scheme", "scheme", new HashSet<string>(StringComparer.OrdinalIgnoreCase) { scheme }, 1);
        public Task<Result<MetadataCandidate>> LookupAsync(NormalizedIdentifier identifier, CancellationToken cancellationToken = default)
            => Task.FromResult(Result<MetadataCandidate>.Failure(MetadataLookupErrorCodes.NotFound, "missing"));
    }

    private sealed class FakeHttpHandler(string response, string contentType, HttpStatusCode statusCode = HttpStatusCode.OK) : HttpMessageHandler
    {
    ...
    {
        public FakeItemService(ItemMetadata item) => Item = item;
        ...
        public Task<Result<ItemMetadata>> UpdateItemAsync(ItemId itemId, UpdateItemRequest request, CancellationToken cancellationToken = default)
        {
            LastUpdate = request;
            Item = Item with { Title = request.Title, Subtitle = request.Subtitle, Publisher = request.Publisher, TagsJson = request.TagsJson ?? "[]", CollectionsJson = request.CollectionsJson ?? "[]", CustomFieldsJson = request.CustomFieldsJson ?? "{}" };
            return Task.FromResult(Result<ItemMetadata>.Success(Item));
        }

        public Task<Result<ItemIdentifier>> AddIdentifierAsync(ItemId itemId, string scheme, string value, string? note, CancellationToken cancellationToken = default)
        {
            var identifier = new ItemIdentifier(IdentifierId.New(), itemId, scheme, value, note, DateTimeOffset.UtcNow);
        ...
        public Task<Result<IReadOnlyList<ItemIdentifier>>> ListIdentifiersAsync(ItemId itemId, CancellationToken cancellationToken = default)
            => Task.FromResult(Result<IReadOnlyList<ItemIdentifier>>.Success(Item.Identifiers));
        public Task<Result> RemoveIdentifierAsync(ItemId itemId, IdentifierId identifierId, CancellationToken cancellationToken = default)
            => throw new NotSupportedException();
        public Task<Result<ItemMetadata>> CreateItemAsync(CreateItemRequest request, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<Result<ItemMetadata>> CreateItemAsync(string itemType, string title, string? subtitle = null, string? titleShort = null, string? creatorsJson = null, string? date = null, string? publicationTitle = null, string? containerTitleShort = null, string? collectionTitle = null, string? publisher = null, string? place = null, string? edition = null, string? genre = null, string? number = null, string? chapterNumber = null, string? volume = null, string? version = null, string? issue = null, string? pages = null, string? language = null, string? status = null, string? note = null, string? abstractText = null, string? tagsJson = null, string? collectionsJson = null, string? customFieldsJson = null, IReadOnlyList<ItemCreatorInput>? creators = null, IReadOnlyList<ItemDateInput>? dates = null, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<Result> DeleteItemAsync(ItemId itemId, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<Result<ItemListPage>> ListItemsAsync(ListItemsRequest request, CancellationToken cancellationToken = default) => throw new NotSupportedException();
    }
}
