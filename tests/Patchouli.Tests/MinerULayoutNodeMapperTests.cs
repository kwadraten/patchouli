...
public sealed class MinerULayoutNodeMapperTests
{
    ...
    {
        MinerUContentListDocument document = new([
        ...
        DocumentInstanceId docId = new(Guid.NewGuid());
        Page[] pages = new[]
        {
            new Page(PageId.New(), docId, 0, null, null, null, 0, "normalized", null, null, "test", null,
                DateTimeOffset.UtcNow, DateTimeOffset.UtcNow),
            new Page(PageId.New(), docId, 1, null, null, null, 0, "normalized", null, null, "test", null,
                DateTimeOffset.UtcNow, DateTimeOffset.UtcNow)
        };

        MinerULayoutNodeMapper mapper = new();
        OcrLayoutDocument layout = mapper.MapDocument(document, pages);
        OcrLayoutBlock[] blocks = layout.Pages.SelectMany(page => page.Blocks).ToArray();
    ...
    public void MapDocument_skips_discarded_blocks()
    {
        ...
        Page[] pages =
        [
            new(PageId.New(), docId, 0, null, null, null, 0, "normalized", null, null, "test", null,
                DateTimeOffset.UtcNow, DateTimeOffset.UtcNow)
        ];
    ...
    public void MapDocument_normalizes_bbox()
    {
        ...
        NormalizedBBox? bbox = layout.Pages[0].Blocks[0].BBox;
    ...
    public void MapDocument_converts_tables_into_row_and_cell_blocks()
    {
        ...
        MinerULayoutNodeMapper mapper = new();
        OcrLayoutDocument layout = mapper.MapDocument(document, pages);
        OcrLayoutBlock table = layout.Pages[0].Blocks.Single();
        ...
        table.Children!.SelectMany(row => row.Children ?? []).Select(cell => cell.Text).Should()
            .Equal("Name", "Value", "Pages", "12");
        table.Children!.SelectMany(row => row.Children ?? []).Where(cell => cell.TableCell!.RowIndex == 0).Should()
            .OnlyContain(cell => cell.TableCell!.IsHeader);
    }
}
