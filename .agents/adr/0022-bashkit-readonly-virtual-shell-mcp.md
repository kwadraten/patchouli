...
- Evidence used opaque `evref` query parameters on text-page URIs. The `evref` token and pinned/current resolution semantics are superseded by ADR `0028`; production evidence URIs use `?rev=&box=`.
