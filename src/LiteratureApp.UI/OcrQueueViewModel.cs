...
public sealed class OcrQueueViewModel : ViewModelBase
{
    ...
    private async Task EnqueueMockAsync()
    {
        await RunAsync(async queue => await queue.EnqueueMockPagesAsync(
            ...
            PageIds.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Select(LiteratureApp.Core.Ids.PageId.Parse).ToArray(),
    ...
    private async Task EnqueueImageAsync()
    {
        await RunAsync(async queue => await queue.EnqueueImagePageAsync(
            LiteratureApp.Core.Ids.DocumentInstanceId.Parse(DocumentInstanceId), OcrPresetId.Parse(PresetId), LiteratureApp.Core.Ids.PageId.Parse(PageId), ImagePath, Priority), "Queued local image OCR task");
    }

    private async Task EnqueueRenderedPdfAsync()
    {
        await RunAsync(async queue => await queue.EnqueueRenderedPdfPageAsync(
            LiteratureApp.Core.Ids.DocumentInstanceId.Parse(DocumentInstanceId), OcrPresetId.Parse(PresetId), LiteratureApp.Core.Ids.PageId.Parse(PageId), int.Parse(Dpi), Priority), "Queued rendered PDF OCR task");
    }
    ...
    private async Task RunAsync(Func<IOcrQueueScheduler, Task<LiteratureApp.Core.Results.Result<OcrQueueTask>>> action, string success)
    {
        try
        {
            var queue = await GetQueueAsync();
