...
namespace LiteratureApp.Infrastructure.Coordinates;

public sealed class PageCoordinateService : IPageCoordinateService
{
    private readonly SqliteConnectionFactory _connectionFactory;
    public PageCoordinateService(SqliteConnectionFactory connectionFactory) => _connectionFactory=connectionFactory;
    public async Task<BBoxConversionResult> ConvertToNormalizedPageAsync(PageId pageId, SourceBBox box, CancellationToken cancellationToken=default)
    {
        if (!ValidNumbers(box)) return Fail(BBoxErrorCodes.Invalid,"BBox values must be finite and width/height positive.");
        var page=await GetPageCoordinateBasisAsync(pageId,cancellationToken);
        if(page.IsFailure) return Fail(BBoxErrorCodes.BasisMissing,page.ErrorMessage!);
        if(box.CoordinateSystem==SourceBBoxCoordinateSystem.NormalizedPage) return ValidateRaw(box.X,box.Y,box.Width,box.Height);
        if(box.CoordinateSystem!=SourceBBoxCoordinateSystem.ImagePixels) return Fail(BBoxErrorCodes.TransformFailed,"OCR source bbox coordinate system is unsupported.");
        var width=box.BasisWidth??page.Value.BasisWidth; var height=box.BasisHeight??page.Value.BasisHeight;
        if(width is null or <=0 || height is null or <=0) return Fail(BBoxErrorCodes.BasisMissing,"Image pixel bbox requires positive basis dimensions.");
        if(page.Value.Rotation is not (0 or 90 or 180 or 270)) return Fail(BBoxErrorCodes.RotationUnsupported,"Page rotation is unsupported.");
        if(box.Rotation!=0 && box.Rotation!=page.Value.Rotation) return Fail(BBoxErrorCodes.BasisMismatch,"Source bbox rotation does not match page rotation.");
        return ValidateRaw(box.X/width.Value,box.Y/height.Value,box.Width/width.Value,box.Height/height.Value);
    }
    public Result ValidateNormalizedBBox(NormalizedBBox bbox) => Result.Success();
    ...
    private static BBoxConversionResult ValidateRaw(double x,double y,double w,double h){if(!double.IsFinite(x)||!double.IsFinite(y)||!double.IsFinite(w)||!double.IsFinite(h)||w<=0||h<=0)return Fail(BBoxErrorCodes.Invalid,"Normalized bbox is invalid.");if(x<0||y<0||x+w>1||y+h>1)return Fail(BBoxErrorCodes.OutOfBounds,"BBox must fit within normalized_page.");return new(true,new NormalizedBBox(x,y,w,h),null,null,BBoxWarning.None);}
    private static BBoxConversionResult Fail(string code,string message)=>new(false,null,code,message,null);
    private sealed class Row{public string PageId{get;set;}="";public string CoordinateBasis{get;set;}="";public double? BasisWidth{get;set;}public double? BasisHeight{get;set;}public int Rotation{get;set;}public string RendererBasisVersion{get;set;}="";public string? SourceFileHash{get;set;}}
    private sealed class WarningRow{public string? SourceFileHash{get;set;}public string? Status{get;set;}public string? Path{get;set;}}
}
