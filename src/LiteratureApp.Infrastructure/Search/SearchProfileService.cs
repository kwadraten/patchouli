...
public sealed class SearchProfileService : ISearchProfileService, IQueryRewriter
{
    ...
    private sealed class Row { public string ProfileId { get; set; } = ""; public string LibraryId { get; set; } = ""; public string Name { get; set; } = ""; public string? Description { get; set; } public int IsSystem { get; set; } public int IsDefault { get; set; } public int Archived { get; set; } public string CreatedAt { get; set; } = ""; public string UpdatedAt { get; set; } = ""; public SearchProfile ToProfile()=>new(SearchProfileId.Parse(ProfileId),LiteratureApp.Core.Ids.LibraryId.Parse(LibraryId),Name,Description,IsSystem!=0,IsDefault!=0,Archived!=0,DateTimeOffset.Parse(CreatedAt),DateTimeOffset.Parse(UpdatedAt)); }
    private sealed class RuleRow { public string RuleId { get; set; } = ""; public string LibraryId { get; set; } = ""; public string? ProfileId { get; set; } public string RuleType { get; set; } = ""; public string Pattern { get; set; } = ""; public string Replacement { get; set; } = ""; public string Direction { get; set; } = ""; public int Enabled { get; set; } public int Priority { get; set; } public string? Note { get; set; } public string CreatedAt { get; set; } = ""; public string UpdatedAt { get; set; } = ""; public SearchRewriteRule ToRule()=>new(SearchRewriteRuleId.Parse(RuleId),LiteratureApp.Core.Ids.LibraryId.Parse(LibraryId),ProfileId is null?null:SearchProfileId.Parse(ProfileId),RuleType,Pattern,Replacement,Direction,Enabled!=0,Priority,Note,DateTimeOffset.Parse(CreatedAt),DateTimeOffset.Parse(UpdatedAt)); }
}
