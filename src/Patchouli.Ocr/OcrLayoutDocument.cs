...
public sealed record OcrLayoutDocument(IReadOnlyList<OcrLayoutPage> Pages)
{
    ...
    {
        if (Pages.Count == 0)
            return Result.Failure(AppErrorCodes.ValidationFailed, "OCR layout document must contain at least one page.");

        var duplicatePageId = Pages
            .GroupBy(page => page.PageId)
            .FirstOrDefault(group => group.Count() > 1);
        if (duplicatePageId is not null)
            return Result.Failure(AppErrorCodes.ValidationFailed, $"OCR layout document contains duplicate page '{duplicatePageId.Key}'.");

        foreach (var page in Pages)
        {
            var pageValidation = ValidateBlocks(page.Blocks);
    ...
    private static int CountBlocks(IReadOnlyList<OcrLayoutBlock> blocks)
        => blocks.Sum(block => 1 + CountBlocks(block.Children ?? []));
    ...
    private static Result ValidateBlocks(IReadOnlyList<OcrLayoutBlock> blocks)
    {
        foreach (var block in blocks)
        {
            ...
            if (string.IsNullOrWhiteSpace(block.TextPolicy) || !TextPolicy.IsKnown(block.TextPolicy.Trim()))
                return Result.Failure(AppErrorCodes.ValidationFailed, $"OCR layout block text policy '{block.TextPolicy}' is invalid.");
            if (block.NodeType == LayoutNodeType.TableCell && block.TableCell is null)
                return Result.Failure(AppErrorCodes.ValidationFailed, "OCR table cell blocks must include table cell metadata.");
            if (block.TableCell is not null)
            {
                if (block.NodeType != LayoutNodeType.TableCell)
                    return Result.Failure(AppErrorCodes.ValidationFailed, "Only table cell blocks may include table cell metadata.");
                if (block.TableCell.RowIndex < 0 || block.TableCell.ColIndex < 0)
                    return Result.Failure(AppErrorCodes.ValidationFailed, "OCR table cell row and column indexes must be non-negative.");
            ...
            var childValidation = ValidateBlocks(block.Children ?? []);
