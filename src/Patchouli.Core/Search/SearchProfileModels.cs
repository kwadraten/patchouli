...
public static class SearchRewriteDirection
{
...
public sealed record SearchRewriteRule(
...
public sealed record SearchProfileSettings(
    LibraryId LibraryId,
...
public sealed record SearchRewriteExpansion(
    string OriginalTerm,
    IReadOnlyList<string> ExpandedTerms,
    IReadOnlyList<SearchRewriteRuleId> RuleIds,
    IReadOnlyList<string> RuleTypes);

public sealed record SearchRewritePlan(
    string OriginalQuery,
    SearchProfileId? EffectiveProfileId,
    string? EffectiveProfileName,
    IReadOnlyList<string> ExpandedQueries,
    IReadOnlyList<SearchRewriteRuleId> AppliedRules,
    IReadOnlyList<string> Warnings,
    bool PreviewOnly);

public sealed record SearchRewriteOptions(
    LibraryId LibraryId,
    SearchProfileId? ExplicitProfileId = null,
    SearchProfileId? SelectedProfileId = null,
    string? Alias = null,
    bool PreviewOnly = false,
    int MaxExpansions = 32,
    bool EnableRegex = true);

public interface ISearchProfileService
{
    Task<Result<SearchProfile>> CreateProfileAsync(string name, string? description,
        CancellationToken cancellationToken = default);

    Task<Result<SearchProfile>> UpdateProfileAsync(SearchProfileId profileId, string name, string? description,
    ...
    Task<Result<SearchRewriteRule>> AddRewriteRuleAsync(SearchProfileId? profileId, string ruleType, string pattern,
        string replacement, string direction, int priority, string? note,
        CancellationToken cancellationToken = default);
    ...
    Task<Result<IReadOnlyList<SearchProfile>>> ListProfilesAsync(bool includeArchived = false,
        CancellationToken cancellationToken = default);

    Task<Result<IReadOnlyList<SearchRewriteRule>>> ListRulesAsync(SearchProfileId? profileId = null,
