...
public sealed class CslItemTypeProfileService : ICslItemTypeProfileService
{
    private static readonly IReadOnlyDictionary<string, CslItemTypeProfile> Profiles =
        new[]
        {
            Create(
                ...
                primaryFields: ["title"],
                recommendedFields: ["author", "issued", "language", "abstract"],
                advancedFields: ["extra_csl"],
                creatorRoles: [ItemCreatorRoles.Author, ItemCreatorRoles.Editor, ItemCreatorRoles.Translator],
                dateRoles: [ItemDateRoles.Issued, ItemDateRoles.Accessed, ItemDateRoles.OriginalDate],
                identifierSchemes: [BuiltInIdentifierSchemes.DOI, BuiltInIdentifierSchemes.ISBN, BuiltInIdentifierSchemes.ISSN],
            ...
            Create("manuscript", "Manuscript", "Archival manuscripts and unpublished material.", ["title"], ["author", "issued"], ["archive_location", "extra_csl"]),
            Create("paper-conference", "Conference Paper", "Conference proceedings papers.", ["title", "author", "container-title"], ["issued", "publisher", "pages", "DOI"], ["extra_csl"]),
            Create("patent", "Patent", "Patent records.", ["title"], ["author", "issued", "number"], ["jurisdiction", "extra_csl"]),
            Create("standard", "Standard", "Standards and specifications.", ["title"], ["publisher", "issued", "number"], ["version", "extra_csl"])
        }
        .ToDictionary(profile => profile.ItemType, StringComparer.Ordinal);

    public Task<Result<IReadOnlyList<CslItemTypeProfile>>> ListProfilesAsync(CancellationToken cancellationToken = default)
        => Task.FromResult(Result<IReadOnlyList<CslItemTypeProfile>>.Success(Profiles.Values.OrderBy(profile => profile.DisplayName, StringComparer.Ordinal).ToArray()));

    public Task<Result<CslItemTypeProfile>> GetProfileAsync(string itemType, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(itemType))
        {
            return Task.FromResult(Result<CslItemTypeProfile>.Failure(AppErrorCodes.ValidationFailed, "Item type is required."));
        }

        return Task.FromResult(
            Profiles.TryGetValue(itemType.Trim(), out var profile)
    ...
    {
        var profile = await GetProfileAsync(itemType, cancellationToken);
