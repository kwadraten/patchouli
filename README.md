...
- OCR 识别错了也不浪费：文本、表格、类型、顺序和 bbox 进入页级不可变 Document Box Tree，PDF 工作台可在草稿中持续校正后再提交。
- Markdown、全文索引、证据引用和 MCP 共享同一 Box Tree；原生 Avalonia 预览使用中央 Markdig 管线，不使用 WebView。
...
- [x] 阻塞任务处理及进度 UI
- [x] 现代化的设置管理及 UI
- [x] 页级 Document Box Tree 与不可变修订
- [x] PDF Box Tree 工作台与原生 Markdown 预览
- [ ] MacOS 适配 （施工中）
- [ ] 冲突处理及 UI（施工中）
- [ ] 类型化题录 （施工中）
- [ ] 扩展 CSL 字段 （施工中）
